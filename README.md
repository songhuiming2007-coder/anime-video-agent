...
人类在 02.5（人审改稿）、03.5（配音顺听）、05（审时间码）与 09（发布）四处介入。

**目录**

- 文档索引：每份文档管什么 → [`docs/INDEX.md`](docs/INDEX.md)
...
```
选题 → 写稿 + 稿件机检 → TTS 配音（云端 GPU / 本地 mlx） → 语义检索与锚点排画面 → [人审时间码]
     → 本地渲染（切片 + 烧字幕 + 混 BGM）→ 11 项自动质检 → 封面候选 + 标题 → [人选并发布]
```
...
| 操作系统 | macOS（开发与渲染） | 本地渲染与交互在 macOS 上开发验证 |
| **算力架构** | 本地 Mac + 云端无头 GPU（可选但推荐） | 架构为「本地便携交互/渲染 + 云端 Headless GPU 算力（AutoDL，ADR-0014/0016）」。云端节点运行 TTS/ASR/VLM/Embedding，本地 Apple Silicon 亦可通过 mlx 运行本地 TTS 与 ASR 兜底 |
...
v2 采用端云解耦架构（ADR-0014/0016）：重型大模型推理上云（AutoDL 无头实例：Qwen3-TTS 1.7B、Qwen3-VL、SenseVoice、bge-m3），视频片源与 ffmpeg 渲染留在本地，彻底解除 16GB 本地内存与发烫瓶颈。本地 Apple Silicon 仍支持 mlx 本地运行。配音引擎已选定 Qwen3-TTS 1.7B（多语种混读，ADR-0017，Index 系列已出局）。

## 2.2 Coding Agent 是前置要求（默认推荐 Pi）

**这个仓库的预期用法是 clone 下来交给成熟的 Coding Agent 驱动**，不是一个人对着终端逐条手敲命令。
实战中默认推荐 [Pi](https://github.com/earendil-works/pi-coding-agent)（配合 Gemini 3.8 / Kimi K3，已稳定生产并发布 15 期全网视频），亦完全兼容 Claude Code 等通用 Agent。
...
## 2.3 需要的模型（本地约 6.4G，云端数据盘约 33GB）
...
没有的东西也是选择：没有第三方 SaaS 商业 API（大模型自托管于云端无头 GPU 实例）、没有数据库、没有编排框架、没有 web 界面、没有一键脚本。
...
**七条数字不相等就不许进每期循环**（v2 起增加第七条意象打标 captions，ADR-0015）。索引或笔记悄悄缩水时，检索照常返回 Top-K、分数照常
...
Phase 0 只攒**候选池**不 定死选曲（2026-08-08 起）：具体每期用哪首，等这期配音（03）出来、
...
分镜支持三类路由通道（ADR-0003/0008/0015）：
- **锚点直通（一等公民）**：`锚点: S01E01 17:50` 或 `[番] SP01 12:30`，直通镜头表时间码，不检索、不占检索分派。异构特典素材（Live/MV/扫图微动）一律走确定性锚点直通（ADR-0010/0011）；
- **台词通道**：`查询: 文本`（纯台词语义），可选加 `人物: 某某` 开启角色在场过滤（软过滤，防说话人正脸错配）；
- **画面语义通道**：`场景: 描述`（VLM 意象打标 captions + bge-m3 文-文检索，ADR-0015，M2b 已解封）。门槛按池/番在 `config/scenes.json` 标定（未标定写了报错）。

**铁律：** 一段只走一个通道，`锚点`、`人物`、`场景` 互斥，混写当场报错；分数只在自己通道内排，绝不跨通道比较。分派时台词段先挑。
