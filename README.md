...
`08-cover.jpg`，外加 `meta.md` 记实际人类耗时）。**看目录里有哪些文件，就知道进行到哪一步。**
...
`01-topic.md` 必须带齐四项：`番`、`模式`（驳论 / 立论）、**`张力`**（一句话）、`锚点`
（季集号），可选 `封面集`。**真正要审的是张力那一句**——它是整条流水线里唯一的编辑判断，
必须在这一步定死并经人批准，不能留到写稿阶段由 agent 自行发明（那等于绕过唯一的人工关卡）。
写稿只校验、不更换。
...
> 这里不再有固定骨架——**写得足够具体的东西，模型都会照抄**。形状从张力里长，skill 只留
> 判据不留模板，判据用「必须有」不用禁令。
...
锚点数、查询是否重复、查询是否误写成画面构图等。主观项（张力、形状是否换过）机器判不了，
脚本会明确提示留给人看。**机检全过不代表能交**，自检两遍顺序不能反：**先保真**（台词
...
11 项，不达标非零退出，不进人工环节：时长 2–4 分钟；音画误差 < 0.5s；响度 -16 LUFS、
...
点一张封面、点一条标题，然后上传。**上传手动做，不自动化。** 在 `meta.md` 记下本期实际
人类耗时。
...
2. 改 `anchor_words`——机检判定「可指认剧情锚点」的具名场合词，逐番不同，不换的话锚点
   检查会永远判 0 处
3. 在 `config/bgm.json` 加一组曲目，每首必须带实测 `lufs`
4. 在 `config/characters.json` 加这部番的角色名表（中文名 ↔ 聚类贴的名字）
5. **重新标定镜头切分阈值**——画风、剪辑节奏逐番不同，沿用上一部的数不算实测
6. 重跑整个 Phase 0。索引按番隔离，`clips` 和 `subindex search` 都按番过滤
