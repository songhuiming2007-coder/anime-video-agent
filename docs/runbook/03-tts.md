...
```bash
ava-cloud push && ava-cloud run <期号> tts && ava-cloud pull
```
...
1. **配音只有首次是全量，后续改动一律增量**：
   ...
   - 某段重录只需删除对应 `seg-XX.wav`，或运行 `python -m pipeline.tts <期> --redo XX`。
2. **多音字错音修正**：通过 `pipeline/g2p.py` 注入拼音，严禁乱动音色参考干声。
