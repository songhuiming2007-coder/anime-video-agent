---
related-issues: D21, D25, N7, N11, N12, N19, N22
related-plans: —
status: accepted
---
...
### 补记（2026-09-08）：云端商业 API (CosyVoice) 评测否决 + CJK 同形字音读泄漏机理与拼音直注治理 (D25)

针对本地 Qwen3-TTS 1.7B 运行时芯片发烫、耗时久以及人工维护 `readings` 错别字替换表边际成本极高的问题，2026-09-08 开展了云端 API 替代方案实测（阿里云百炼 CosyVoice 声音复刻，同等参考干声），并对发音错乱根因做出了底层定性：

#### 1. 云端商业 API 实测否决：字正腔圆但丧失艺术灵魂
- **实测现象**：云端 CosyVoice 多音字与外文词发音稳定性极佳，但听感「毫无感情、冰冷面瘫，且带明显金属编码伪影与频噪」，与本地跑出的声音「隔着十万八千里」；
- **底层根因**：商业云服务为追求万级高并发与低延迟，对模型进行了激进的剪枝量化、动态降噪与有损压缩，把换气、叹息、喉音等声学微动态作为「噪声」过滤，抹杀了严肃纪实长视频的核心叙事张力；而本地 1.7B Base 跑的是全参数无损自回归声学渲染，具备不可替代的「贴耳感与声情并茂」；
- **决策结论**：**坚决否决云端 API 替代路线，确立本地 Qwen3-TTS 1.7B 为不可妥协的声学质量护城河**。

#### 2. 「逆天错别字替换表」的本质根因：CJK 同形字日语音读泄漏 (Cross-lingual On'yomi Bleed)
- 历史积累的 readings 映射（世界→逝戒/秀界、全部→权部/zenbu、早期→藻期/sōki、监督→坚督/kantoku、舞台→舞抬/butai、五百→佰/gohyaku）具有高度一致性；
- **根因定性**：Unicode 中 CJK 统一表意文字的中日汉字在 Tokenizer 中共用 Token ID。Qwen3 作为中日英多语种自回归底模，在上下文微小漂移时，注意力头会发生跨语种声学借调，激活日语汉字的音读/训读隐空间；
- **治理路线（D25）**：人工逐词打地鼠边际成本无限高，未来唯一正确的收敛路径是**拼音直注脱敏（Pinyin Injection）**——拼音无日语语义/声学映射，可物理阻断日语音读借调（先例：`shí六首`、`pèi乐`、`shēng前` 实测 100% 咬死）。后续规划自动化 G2P 预检机制替代人工错别字映射。
