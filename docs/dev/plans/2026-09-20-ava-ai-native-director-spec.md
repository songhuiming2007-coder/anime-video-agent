...
日期：2026-09-20（**v1.4，四轮红队终审通过，待开工令**）
...
状态：**设计已闭合，等用户开工令；获准前禁止动 `pipeline/` 任何一行**
...
6. **审批事件记账**：每张卡片的 {时间, 工具名, 规范化命令/文件名, y/n} 追加一行
   到期目录 `_agent/approvals.jsonl`（宿主编外文件，不进读域，三行代码的事）——
   它是 §7-1 审批疲劳止损判据的唯一读数来源（v1.1 🟡 修正：没有读数的判据
   就是死判据，与 v1.20 Y1-r19 同一个病）。
...
- 每条变异单独施加、单独跑、单独恢复，记录「变异 → 红条数」映射进 PR 描述
  （沿用 PR4「变异验证 4/4 命中」的记账格式）；
