...
> 状态：**已完成**（2026-09-07）。完成判定全部满足，V1–V5 验证全 OK。
> 执行实录：① 源目录实际混入 `PSYCHO-PASS II [06]`（S2 第 6 集，不在范围内），迁移按 `PSYCHO-PASS [NN]` 精确过滤，S2 文件留在待入库——若按原 glob 全搬，phase0 会把它登记成 S01E06 顶掉正片；② SP04/SP05 首次 build 发现集键覆盖只记 meta 不参与切分（build 里 cut 与 meta 各调一次 threshold），已修为 meta 单点取值并重切（commit 74cd64b）。
