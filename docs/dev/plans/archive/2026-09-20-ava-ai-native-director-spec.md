...
基线（含 M21 与 harness 自检测试）为 **1440 passed**。

**一处计数假的修正**：harness 落库时把「矩阵锚点是否逐字命中」的**反向自检**也放进了
逐条计分的套件里——而变异恰好会替换掉锚点文本，于是它必然报红，**每条变异白涨 1 条**
（M1 一度被记成 15）。它不是护栏被绕过的证据，已从逐条套件里 `--deselect`（由 harness
自己在开跑前把关），并抽 M1/M14/M17/M10 复验计数回落。顺带收紧 M10 锚点：`= 10` 是
`= 100` 的前缀子串，不带换行的锚点在变异后仍「命中」，会漏报过期锚点——**子串判据会
把「护栏还在」这句话变成假的**。
