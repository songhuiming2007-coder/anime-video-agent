...
| D25 | 待决策 | readings 替换表边际成本极高，需将 CJK 音读泄漏收敛为拼音直注脱敏机制 | `config/voice.json`, `pipeline/tts.py` | ADR-0006 | 2026-09-08 实测否决云端 API（声音冰冷面瘫）；确立念错根本机理为 CJK 汉字日语音读泄漏，后续需将手工错别字重构为拼音直注自动化预检 |
