...
| B2 | 阻塞 | ≤10 分钟止损线已被实际超限，改线 / 降耗时未定 | `WORKFLOW.md:631-634` | P4, ADR-0014 | 02.5 实测约 60 min；P4 机制已落地但净耗时下降无证据，需继续计时 |
...
| D4 | 待决策（触发式） | 视觉索引第 3 层 VLM 启动条件挂起 | `docs/adr/0003:38-39,95-141` | ADR-0003, ADR-0014 | 启动条件：前两层跑 ≥3 期 + 失败原因确为「需理解画面」而非查询错 |
...
| D22 | 待决策 | 泛素材多模态漏斗检索（Top-K 候选轻量视觉意象过滤）待探针设计 | `docs/adr/0005` | B1, D1, D4, D5, ADR-0014 | BGE embedding 字面相似≠视觉意象；需验证在 Top-10 代表帧做轻量 VLM 过滤的漏斗机制 |
...
| D25 | 待决策 | readings 替换表边际成本极高，需将 CJK 音读泄漏收敛为拼音直注脱敏机制 | `config/voice.json`, `pipeline/tts.py` | ADR-0006, ADR-0014 | 2026-09-08 实测否决云端 API（声音冰冷面瘫）；确立念错根本机理为 CJK 汉字日语音读泄漏，后续需将手工错别字重构为拼音直注自动化预检 |
