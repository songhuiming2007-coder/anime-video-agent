...
|---|---|---|---|---|---|
| B1 | 阻塞 | 排片错配修复方案未选定 | `docs/adr/0005:4-6` | ADR-0005, P1, v2重构方案 | 探针工具就绪（`pipeline/recheck.py`）；v2 云端 Qwen2-VL 全镜头意象打标为终极解法 |
...
| D4 | 待决策（触发式） | 视觉索引第 3 层 VLM 启动条件挂起 | `docs/adr/0003:38-39,95-141` | ADR-0003, ADR-0014, v2重构方案 | 启动条件满足（纪录片台词脱节证明必须理解画面）；收束进 v2 云端 GPU 重构方案 |
| D5 | 待决策（长期） | 第 2 层（画面语义）复活的两条路未试 | `docs/adr/0003:431-433` | ADR-0003, v2重构方案 | 弃用 CLIP 余弦，由 v2 云端 Qwen2-VL 统一收束实现镜头意象打标 |
...
| D22 | 待决策 | 泛素材多模态漏斗检索（Top-K 候选轻量视觉意象过滤）待探针设计 | `docs/adr/0005` | B1, D1, D4, D5, ADR-0014, v2重构方案 | BGE 字面相似≠视觉意象；收束进 v2 方案由 Qwen2-VL 逐镜头打标代替漏斗粗滤 |
| D23 | 待决策 | 配音顺听（工序 B）人机摩擦力大，缺终端流式连播与原地纠偏工作台 | `pipeline/tts.py` | 四阶段工序卡, ADR-0006, v2重构方案 | v2 云端 7B Flow Matching TTS 0 错读后，摩擦力从根本上消除 |
...
| D25 | 待决策 | readings 替换表边际成本极高，需将 CJK 音读泄漏收敛为拼音直注脱敏机制 | `config/voice.json`, `pipeline/tts.py` | ADR-0006, ADR-0014, v2重构方案 | 2026-09-08 确立根因为 CJK 汉字音读泄漏；v2 上 7B 大模型解耦 + 拼音直注彻底废除手工错别字表 |
