...
|---|---|---|---|---|---|
| B1 | 已定案，v2 推进中 | 排片错配修复方案未选定 | `docs/adr/0005:4-6` | ADR-0005, ADR-0015, P1, v2架构方案 | 定案为 Qwen3-VL 镜头意象打标 + bge-m3 文-文检索（ADR-0015）；探针工具（`pipeline/recheck.py`）继续用于 M2/M3 错配率复测 |
...
| D4 | 已定案（ADR-0015），v2 推进中 | 视觉索引第 3 层 VLM 启动条件挂起 | `docs/adr/0003:38-39,95-141` | ADR-0003, ADR-0015, ADR-0014, v2架构方案 | 启动条件已满足（纪录片台词脱节证明必须理解画面）；落地形态 = ADR-0015 的 VLM 打标 + 文-文检索，M2 验收 |
| D5 | 已定案（ADR-0015），v2 推进中 | 第 2 层（画面语义）复活的两条路未试 | `docs/adr/0003:431-433` | ADR-0003, ADR-0015, v2架构方案 | 走第二条路：VLM 判断——以 Qwen3-VL captions + bge-m3 文-文检索实现，CLIP 余弦正式废弃，M2 验收后解封 |
...
| D23 | 已定案，v2 推进中 | 配音顺听（工序 B）人机摩擦力大，缺终端流式连播与原地纠偏工作台 | `pipeline/tts.py` | 四阶段工序卡, ADR-0006, ADR-0016, v2架构方案 | 定案为云端 TTS 情感/时长控制（选型见 ADR-0017：Index 系已出局，CosyVoice/Qwen3 评审中）+ 03.5/05 三十秒结构化打点（v2架构方案二.3/三节） |
...
| D25 | 已定案，v2 推进中 | readings 替换表边际成本极高，需将 CJK 音读泄漏收敛为拼音直注脱敏机制 | `config/voice.json`, `pipeline/tts.py` | ADR-0006, ADR-0014, v2架构方案 | 定案为 `pipeline/g2p.py` 拼音直注脱敏层（历史泄漏音素集 RISKY + 闭环回读验证），readings 表降级为个例 override；M2 验收后连表删除 |
