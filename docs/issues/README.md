...
| N27 | 备忘 | 伪恋/夏隧的 scene_threshold=10.0 是沿用值，标定实录缺失 | `config/project.json` visual | ADR-0003 | 2026-09-05 审计发现；两番镜头表均已建（伪恋 36 集且已出片），补跑 calibrate 验证，若改值对受影响集 shots rebuild（免重解码） |
