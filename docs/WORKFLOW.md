...
| **B. 配音与顺听** | 03 语音合成 | `python -m pipeline.tts <期>` | **03.5 配音顺听**：顺听 + /voice 纠错（可选深挖，corrections.json 永久资产，--apply-patch 靶向重配）。 |
...
5. **[03.5 顺听](runbook/03.5-voice-check.md)**（人）：顺听 + /voice 纠错（可选深挖；三项抽检为主，corrections.json / --apply-patch）。
...
1. **配音只有首次是全量**：后续改动一律走点名重配（`/voice` 纠错 → `--apply-patch` 或 `--redo`），严禁 `--force` 或擅自换引擎洗掉全量音频。
